...
The Hubmap Entity Python library provides convenient access to the Hubmap Entity REST API from any Python 3.8+
...
pip install git+ssh://git@github.com/yuanzhou/test-sdk.git
```
...
Simply import `AsyncHubmapEntity` instead of `HubmapEntity` and use `await` with each API call:
...
When the library is unable to connect to the API (for example, due to network connection problems or a timeout), a subclass of `hubmap_entity.APIConnectionError` is raised.

When the API returns a non-success status code (that is, 4xx or 5xx
response), a subclass of `hubmap_entity.APIStatusError` is raised, containing `status_code` and `response` properties.

All errors inherit from `hubmap_entity.APIError`.
...
except hubmap_entity.RateLimitError as e:
    print("A 429 status code was received; we should back off a bit.")
except hubmap_entity.APIStatusError as e:
...
You can enable logging by setting the environment variable `HUBMAP_ENTITY_LOG` to `info`.

```shell
$ export HUBMAP_ENTITY_LOG=info
```
...
```python
import httpx
from hubmap_entity import HubmapEntity, DefaultHttpxClient

client = HubmapEntity(
    # Or use the `HUBMAP_ENTITY_BASE_URL` env var
...
```py
import hubmap_entity
print(hubmap_entity.__version__)
```
